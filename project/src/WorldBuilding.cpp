...
void WorldBuilding::initCity(string configFile)
{
    ...
    //cout<<castle->m_mapCoordinates.x<<" "<<castle->m_mapCoordinates.y<<" "<<castle->m_img<<" "<<castle->m_objectTexture<<endl;
    ///initBuildings("buildings.txt");
}
...
void WorldBuilding::saveBuildings(string configFile)
{
    ...
    {
        in_file << (*it)->m_type << "; X: ";
        in_file << (*it)->m_mapCoordinates.x << "; Y: ";
        in_file << (*it)->m_mapCoordinates.y<< "\n";
    }
    in_file.close();
...
void WorldBuilding::loadBuildings(string configFile)
{
    ...
    {
        in_file >> buff;
        building -> m_type = buff;

        if(buff == "Buildings\\castle.txt"){

            building = new Castle(*(world.m_configManager.modelCastle));
        }
        if(buff == "Buildings\\street.txt"){

            building = new Building(*(world.m_configManager.modelStreet));
        }
        if(buff == "Buildings\\archery.txt"){

            building = new Building(*(world.m_configManager.modelArchery));
        }
        if(buff == "Buildings\\shop.txt"){
        ...
        in_file >> buff;
        building->m_mapCoordinates.y = val;
...
void WorldBuilding::selectTile()
{
    ...
    /*if (world.m_event.type == SDL_MOUSEMOTION)
    {
        ...
        world.m_mouse.x *= world.m_MOUSE_MULTIPLIER_X;
        world.m_mouse.y *= world.m_MOUSE_MULTIPLIER_Y;
    }*/
    showUI();
    building();
}
